          ...
          {products.map((product) => (
            ...
            >
              <div className="overflow-hidden rounded-lg border bg-white">
                ...
                <div className="p-4">
                  <h3 className="line-clamp-1 text-sm font-medium">{product.name || "Unnamed Product"}</h3>
                  <div className="mt-1 flex items-center justify-between">
                    <div>
                      {(product.originalPrice || product.original_price) ? (
                        <div className="flex items-center gap-2">
                          <span className="font-medium">KSh {parseFloat(product.price || 0).toFixed(2)}</span>
                          <span className="text-xs text-gray-500 line-through">
                            KSh {parseFloat(product.originalPrice || product.original_price || 0).toFixed(2)}
                          </span>
                        </div>
                      ) : (
                        <span className="font-medium">KSh {parseFloat(product.price || 0).toFixed(2)}</span>
